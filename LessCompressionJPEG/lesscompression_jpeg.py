...
def zigzag_points(rows, cols):
    ...
    def move(direction, point):
        return {UP: lambda point: (point[0] - 1, point[1]),
...
def encode():
    input_file = '12_3.bmp'
    output_file = 'Results/12_3_t2.asf'
    ...
    H_AC_Y = HuffmanTree(flatten(run_length_encode(ac[i, :, 0])
                                 [0] for i in range(blocks_count)))

    H_AC_C = HuffmanTree(flatten(run_length_encode(ac[i, :, j])
                                 [0]for i in range(blocks_count) for j in [1, 2]))
    ...
    global size_vyhsdnogo
    size_vyhsdnogo = os.path.getsize(input_file)
    with open("results_jpeg.txt", "a", encoding='utf-8') as file:
        print('Дані для слаботекстурного зображення RGB (таблиця квантування 2)', file=file)
...
def decoder():
    dc, ac, tables, blocks_count = read_image_file("./Results/12_3_t2.asf")
    ...
    filename = f"./Results/JPEG_{'12_3_t2'}.jpg"
    image.save(filename)
    ...
    with open("results_jpeg.txt", "a", encoding='utf-8') as file:
        print('Розмір файла JPEG: {} байт'.format(size_jpeg), file=file)
